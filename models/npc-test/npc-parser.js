/**
 * Korean scenario parser — facts[] for QuestSystem (no DOM).
 * Requires dictionaries.js (window.NpcLexicon) loaded first.
 */
(function (global) {
  var _lexCache = null;
  function getLexiconVars() {
    if (_lexCache) return _lexCache;
    var lexicon = global.NpcLexicon || {};
    _lexCache = {
      KNOWN_ACTIONS: lexicon.knownActions || [],
      ACTION_PATTERNS: lexicon.actionPatterns || [],
      NON_COUNTABLE_PATTERNS: lexicon.nonCountablePatterns || [],
      CLASSIFY_KEYWORDS: lexicon.classifyKeywords || {},
      ACTION_NORMALIZE_MAP: lexicon.actionNormalizeMap || {},
      WORLD_PLACES: lexicon.worldPlaces || [],
      WORLD_ROLES: lexicon.worldRoles || [],
    };
    return _lexCache;
  }
  ...
  var KNOWN_ACTIONS,
    ACTION_PATTERNS,
    NON_COUNTABLE_PATTERNS,
    CLASSIFY_KEYWORDS,
    ACTION_NORMALIZE_MAP,
    WORLD_PLACES,
    WORLD_ROLES;

  function bindLexiconVars() {
    var L = getLexiconVars();
    KNOWN_ACTIONS = L.KNOWN_ACTIONS;
    ACTION_PATTERNS = L.ACTION_PATTERNS;
    NON_COUNTABLE_PATTERNS = L.NON_COUNTABLE_PATTERNS;
    CLASSIFY_KEYWORDS = L.CLASSIFY_KEYWORDS;
    ACTION_NORMALIZE_MAP = L.ACTION_NORMALIZE_MAP;
    WORLD_PLACES = L.WORLD_PLACES;
    WORLD_ROLES = L.WORLD_ROLES;
  }
  bindLexiconVars();
  ...
  global.NpcParser = {
    ...
    detectRumorText: detectRumorText,
    pickKoreanParticle: pickKoreanParticle,
    sanitizeKoreanNoun: sanitizeKoreanNoun,
    normalizeActionPhrase: normalizeActionPhrase,
    resetLexiconCache: function () {
      _lexCache = null;
      bindLexiconVars();
    },
  };
