/**
 * 밀수 화물 퀘스트 — 3 의뢰인 × outcome 분기 리포트
...
const REPORT_TEXT = "안개 계곡에 밀수 화물이 버려져 있다. 마약초 열두 개가 들어 있다";
...
function main() {
  ...
  for (const giverId of GIVERS) {
    const run = runtime.runQuestTurnIn({
      ...
      engine,
      parser,
      sessionKey: "cargo-slice-" + giverId,
      reputationSessionKey: "cargo-slice-" + giverId,
    });
  ...
  const report = {
    ...
    parserFacts,
    paths,
    note: "branch authority_path: qty>=10 & certainty>=0.6 | black_market_path: qty<=9",
  };
