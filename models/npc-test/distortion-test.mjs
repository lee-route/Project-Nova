...
// Reuse browser evaluator logic inline
...
function evaluateCase(testCase, parser, engine) {
  ...
  if (testCase.custom === "kb_merge") {
    ...
    const list = kb.list();
    if (mergeRes.action !== "merge" || list.length !== 1 || list[0].truth_value.quantity < 7) {
...
function main() {
  ...
  for (const testCase of suite.cases) {
    const res = evaluateCase(testCase, parser, engine);
    if (res.ok) {
      pass += 1;
      console.log("[PASS]", testCase.id);
    } else {
      fail += 1;
      console.log("[FAIL]", testCase.id, "->", res.reason);
    }
  }
